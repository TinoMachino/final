...
export async function createAgentAndResume(
  ...
  let serviceUrl = storedAccount.service
    ...
    super({
      ...
      async fetch(...args) {
        ...
        try {
          ...
          // Check for "User not found" error which indicates a stale session (e.g. after PDS reset)
            ...
            try {
              ...
              if (
                ...
                (body?.message === 'Could not find user info' ||
                  body?.message?.startsWith('Could not find user info'))
              ) {
                emitSessionDropped()
              }
            } catch (e) {
