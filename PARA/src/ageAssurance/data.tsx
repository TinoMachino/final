...
const persister = createAsyncStoragePersister({
  storage: createPersistedQueryStorage('age-assurance-query-client'),
...
export function getServerStateFromCache({
  ...
  did: string
}): AppBskyAgeassuranceGetState.OutputSchema | null | undefined {
  return qc.getQueryData<AppBskyAgeassuranceGetState.OutputSchema | null>(
...
export async function refetchServerState({agent}: {agent: AtpAgent}) {
  ...
  const res = await networkRetry(3, () => getServerState({agent}))
  qc.setQueryData<AppBskyAgeassuranceGetState.OutputSchema | null>(
    createServerStateQueryKey({did}),
    res,
  )
  return res
